    ...
    <div class="container filter-wrapper">
        <form action="" method="GET" class="row g-3 align-items-end" autocomplete="off">
            ...
            <div class="col-md-2 d-flex gap-2">
                ...
                @if (!empty(request()->position) || !empty(request()->agent))
                    <a href="{{ route('admin.ads.pending') }}" class="btn btn-danger">Clear</a>
                @endif
            </div>
        </form>
    </div>
        ...
        <table class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <th style="width: 5%;">Applied On</th>
            ...
            <tbody>
                @forelse ($ads as $item)
                    <tr>
                        ...
                        <td style="width: 5%;">
                            {{ $item->formatted_date('applied_on') }}
                        </td>
                        <td style="width: 10%;">
                        ...
                        <td style="width: 30%;">
                            <div class="d-flex gap-2">
                                <button class="btn btn-primary show_details"
                                    data-url="{{ route('admin.ads.show.details', ['id' => $item->id]) }}">
                                    View
                                </button>
                                <button class="btn btn-success confirmation"
                                    data-url="{{ route('admin.ads.approve', ['id' => $item->id]) }}"
                                    data-text="Are you sure you want to approve this ad?">
                                    Approve
                                </button>
                                <button class="btn btn-danger confirmation"
                                    data-url="{{ route('admin.ads.reject', ['id' => $item->id]) }}"
                                    data-text="Are you sure you want to reject this ad?">
                                    Reject
                                </button>
                            </div>
                        </td>
    ...
    @if (!empty($id))
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                let url = "{{ route('admin.ads.show.details', ['id' => $id]) }}";
                $.ajax({
                    type: "GET",
                    url: url,
