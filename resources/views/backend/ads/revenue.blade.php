...
@section('content')
    ...
    <div class="container filter-wrapper">
        ...
        <form action="" method="GET" class="row g-3 align-items-end" autocomplete="off">
            ...
            <div class="col-md-2">
                <label for="payment_date_to" class="form-label">Payment Date From</label>
                <input type="date" name="payment_date_from" id="approved_date_from" class="form-control"
                    value="{{ request()->payment_date_from ?? '' }}">
            </div>
            <div class="col-md-2">
                <label for="payment_date_to" class="form-label">Payment Date To</label>
                <input type="date" name="payment_date_to" id="approved_date_to" class="form-control"
            ...
            <div class="col-md-2 d-flex gap-2">
                ...
                @if (
                    !empty(request()->position) ||
                        ...
                        !empty(request()->payment_date_from) ||
                        !empty(request()->payment_date_to))
                    <a href="{{ route('admin.ads.revenue') }}" class="btn btn-danger">Clear</a>
                @endif
            </div>
        </form>
    </div>
                ...
                @forelse ($ads as $item)
                    <tr>
                        ...
                        <td style="width: 5%;" class="text-center">
                            {{ format_amount($item?->rate) }}/day
                        </td>
                        <td style="width: 10%;" class="text-center">
                            {{ $item?->total_days ?? '' }} {{ $item?->total_days == 1 ? 'day' : 'days' }}
                        </td>
                        <td style="width: 10%;" class="text-center">
                            <span class="text-success">{{ format_amount($item?->amount) }}</span>
                        </td>
                    </tr>
                @empty
                    <tr>
